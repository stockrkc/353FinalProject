...
bool gp_timer_wait(uint32_t base_addr, uint32_t ticks)
{
  ...
  // ADD CODE
  //*********************
	gp_timer->CTL &= ~TIMER_CTL_TAEN; // Stop Timer A
	gp_timer->CTL &= ~TIMER_CTL_TBEN; // Stop Timer B
	gp_timer->TAILR = ticks; // Set interval of Timer A
	
	// Clear Timer A timeout
	gp_timer->ICR |= TIMER_ICR_TATOCINT;
	
	gp_timer->CTL |= TIMER_CTL_TAEN; // Start Timer A
	gp_timer->CTL |= TIMER_CTL_TBEN; // Start Timer B
	
	// Busy-wait until Timer A timeout occurs
	while ((TIMER_RIS_TATORIS & gp_timer->RIS) == 0) {
	}
...
bool gp_timer_config_32(uint32_t base_addr, uint32_t mode, bool count_up, bool enable_interrupts)
{
  ...
  //*********************
    gp_timer->CTL &= ~(TIMER_CTL_TAEN | TIMER_CTL_TBEN);
		...
		// Set size of timer to 32-bits wide
    ...
    gp_timer->TAMR |= mode;
		...
		// Enable/disable Timer A timeout interrupts
